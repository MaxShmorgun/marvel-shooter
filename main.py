...
class PlayerShip(Ship):
    ...
    def animation(self, dt):
        if self._lastAnim >= self.anim_delay:
            p = Particle(
                source="images/particle_real.png",
                width = dp(70 + randint(0, 50)),
                center_x=dp(self.center_x + randint(-10, 10)),
...
class GameScreen(MDScreen):
    def __init__(self, *args, **kwargs):
        ...
        self.backBack = MoveBackground(source='images/cosmos.png', speed=0.2)
        self.backFront = MoveBackground(source='images/planets.png', speed=1, scale=3)

        self.ids.back.add_widget(self.backBack)
        self.ids.back.add_widget(self.backFront)

        # Частинки для ефектів
        self.particles = []
